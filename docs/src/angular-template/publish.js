...
var getChildren = function(data, doclet) {
  ...
  var children = {};
  var childrenWithLongname={};
  members.forEach(function(doclet) {
    // Get rid of weird namespaces
    if(doclet.name.indexOf('[undefined]')!==-1) return;
    children[doclet.kind] = children[doclet.kind] || [];
    children[doclet.kind].push(doclet);
    childrenWithLongname[doclet.kind] = childrenWithLongname[doclet.kind] || {};
    childrenWithLongname[doclet.kind][doclet.longname] = childrenWithLongname[doclet.kind][doclet.longname] || [];
    childrenWithLongname[doclet.kind][doclet.longname].push(doclet);
  });

  Object.keys(childrenWithLongname).forEach(function(kind){
    Object.keys(childrenWithLongname[kind]).forEach(function(longname){
      var allDups=childrenWithLongname[kind][longname];
      if(allDups.length<2) return;
      var sortImportant=allDups.sort(function(prev,doclet){
        var importance = 0;
        if (!doclet.type) importance-=2;
        if (doclet.comment) importance +=1;
        return importance;
      });
      sortImportant.forEach(function(doclet,i){
        if(i>0) children[doclet.kind].splice(children[doclet.kind].indexOf(doclet),1);
      });
    });
  });
  return children;
